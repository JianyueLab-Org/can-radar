...
**预约卡里的「个人预约」那一段**（`RadarBookings.vue`）是这个站点唯一**匿名看不
到**的东西。板子上有成员用户名和自由填写的备注，那是网络内部的；上游那条
（can-api 的 `GET /api/v1/atc/reservations`）也确实挂在 `WithPilot` 后面。所以本
站那条同名转发（`src/pages/api/v1/atc/reservations.ts`）**没有 cookie 就答 401**，
一次上游都不打，而没登录时那一段**连标题都不出现** —— 不是出现之后写一句劝人登
录的话。要让它公开的话，改的是 can-api 那条路由的守卫，不是这里加个绕过。

**同一张卡上的「活动席位」那一段是公开的**，别把上面那条规矩顺手套到它头上：它
来自 can-api 的活动接口，那两条没有守卫（一场活动是公告出去的东西）。**一张卡两
段、两种可见性，是两个上游本来就不同的答案**，不是谁忘了统一 —— `lib/schedule.ts`
顶上把这件事完整写了一遍。活动卡（`RadarActivities.vue`）同理，人人可见。

## 活动板是唯一一条不是「一对一转发」的路由

`/api/v1/track`、`/api/v1/metar`、`/api/v1/atc/reservations` 都是一条路由对一条上
游。`/api/v1/activity/board` 不是：can-api 的列表只给出每场活动**有几个席位、还剩
几个空**，谁预约了哪个席位要一场一场去问 `/api/v1/activity/{id}`。而预约卡上「活
动席位」那一段要的正是后者。

让浏览器自己扇出是不行的 —— 全网最公开的这一页每打开一次就发 1+N 个请求。所以扇
出收在服务端（`src/server/activityBoard.ts`），浏览器只发一个。三件配套的事，少哪
一件这条路由都会变成打 can-api 的锤子：

- **60 秒的进程内缓存**，外加一个 **in-flight 合流**（同一时刻涌进来的请求共用同
  一次扇出）。进程内的变量而不是 Redis，理由和 `rateLimit.ts` 那处是同一条：单进
  程的 standalone Node；真要多副本时换掉的是那个文件，不是调用方。
- **扇出有上限**（`MAX_FANOUT`）。撞到上限时多出来的活动**仍然在活动卡上**，只是
  它们的席位不进预约卡 —— 列表那一份是完整的，被截断的只有扇出。
- **不带 cookie。** 上游没有守卫，但带上会话会让答复因人而异（can-api 给 SUP 多返
  回草稿），那样就既不能缓存也不能带 `public` 的缓存头。草稿因此在雷达上永远不出
  现，这正是想要的。

失败的分寸也在那儿：**列表取不到就整块失败**（卡片写「取不到」而不是「近期没有活
动」），**单场活动取不到只是少了它的席位**，不牵连整块板子。
...
（`RadarTraffic`）、活动卡（`RadarActivities`）和预约卡（`RadarBookings`）。分开
是因为两者常常要同时看 —— 挤在同一列时，选中一架飞机会把名单整个推下去，而人多
半正想在名单里挑下一架。

**那一摞的顺序是一条时间轴：现在 → 将要 → 具体是谁。** 交通说的是现在谁在线，活
动说的是将要发生什么事，预约说的是那些事和别的时段各会有谁开席。活动排在预约上
面，是因为它是那些席位的由头 —— 先看到「周六有连飞」，下面那一排 ZBAA_TWR、
ZSSS_APP 才有意思；而两者都排在交通下面，因为一张多半为空的板子不该挡在最要紧的
名单前面。

后两张卡**没有内容时整张不出现**（近期没有活动 / 既没登录也没有活动席位）：一张
永远空着的卡压在全网最公开的这一页上，是每个人每次都要滑过去的一格噪音。
