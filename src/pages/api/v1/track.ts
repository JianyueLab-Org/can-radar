 ...
 * 飞过的航迹 —— 一个转发，不是数据源。
 *
 * 航迹存在 `flightPosition` 表里，那张表归 can-web 的 Prisma 模型所有，由
 * can-fsd 写入和清理。雷达拆出来的时候**只有这一件事**没有跟着搬：搬了就意味
 * 着这个站点要拿着数据库口令，而它是整个网络上最公开、最没有登录、被爬得最凶
 * 的一个页面。让最暴露的东西持有数据库凭据，是这次拆分最不该做的事。
 *
 * 所以这里在服务端去问 can-web，浏览器那边仍然是同源的一次 fetch。
 * 顺带的好处是**不用给 can-web 开 CORS** —— 少改一个仓库，也少一个跨源的口子。
 *
 * 两处限流不是重复：这一桶挡的是打到我们这儿的量，can-web 那边还有它自己的
 * 一桶挡我们。真正的天花板是那一个。
 */

const UPSTREAM = (process.env.CAN_WEB_ORIGIN || "https://airwaysn.org").replace(
  /\/+$/,
  "",
);
